...
export default function NewOrderPage() {
  ...
  function handleFinishingToggle(id: string) {
    ...
    if (isFuro) {
      ...
      if (selectedFinishing.includes(id)) {
        setSelectedFinishing([])
      } else {
        ...
        const withoutFuros = selectedFinishing.filter(
          (fid) => !onlyFuros.includes(fid)
        )

        setSelectedFinishing([...withoutFuros, id])
      }
    ...
    if (selectedFinishing.includes(id)) {
      setSelectedFinishing(selectedFinishing.filter((f) => f !== id))
    } else {
      setSelectedFinishing([...selectedFinishing, id])
    }
  }
  ...
  const handleSubmitOrder = async () => {
    const currentUser = getCurrentUser()
    if (!currentUser || !selectedProduct || !artworkFile || !artworkValidation)
      return

    setIsProcessing(true)

    const material = selectedProduct.materials.find(
  ...
  const canProceedStep1 = selectedProduct && selectedMaterial
  const canProceedStep2 = artworkFile && artworkValidation
  ...
  return (
    <DashboardLayout
      ...
      description='Follow the steps to create your print order'
    >
      {/* Progress Bar */}
      <div className='mb-8'>
          ...
          <CardContent className='space-y-6'>
            ...
            <div>
              ...
              <div className='space-y-3'>
                {getCompatibleFinishing(
                  ...
                  >
                    <Checkbox
                      ...
                      onCheckedChange={() =>
                        handleFinishingToggle(finishing.id)
                      }
                    />
                    <div className='flex-1'>
                      <Label
                ...
                {getCompatibleFinishing(selectedMaterial, selectedProduct.id)
                  .length === 0 && (
                ...
                <div className='space-y-4'>
                  ...
                  <div>
                    ...
                    <p className='font-medium'>
                      {
                        selectedProduct.materials.find(
                          (m) => m.id === selectedMaterial
                        )?.name
                      }
                    </p>
