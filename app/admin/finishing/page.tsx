...
import { FINISHING_OPTIONS, PRODUCTS, type FinishingOption } from "@/lib/products"

export default function AdminFinishingPage() {
  const router = useRouter()
  const [isDialogOpen, setIsDialogOpen] = useState(false)
  const [editingFinishing, setEditingFinishing] = useState<FinishingOption | null>(null)
  ...
  const getProductsUsingFinishing = (finishingId: string) => {
    return PRODUCTS.filter((product) =>
      product.materials.some((material) => material.compatibleFinishing.includes(finishingId)),
    )
  }

  return (
    <DashboardLayout title="Finishing Options" description="Manage finishing options and product compatibility">
      <div className="flex justify-between items-center mb-6">
        <div>
          <h2 className="text-2xl font-bold">Finishing Options</h2>
          <p className="text-muted-foreground">Create and manage finishing options available for products</p>
        </div>
        <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
          ...
          <DialogContent>
            <DialogHeader>
              <DialogTitle>{editingFinishing ? "Edit Finishing Option" : "Create New Finishing Option"}</DialogTitle>
              <DialogDescription>
                {editingFinishing ? "Update finishing option details" : "Add a new finishing option to the catalog"}
              </DialogDescription>
            </DialogHeader>
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="name">Option Name</Label>
                <Input id="name" defaultValue={editingFinishing?.name} placeholder="Lamination" />
              </div>
              ...
              <div className="space-y-2">
                ...
                <div className="border rounded-lg p-3 bg-muted/50">
                  <p className="text-sm text-muted-foreground">
                    Finishing options are associated with specific product materials. Manage compatibility in the
                    Products page.
                  </p>
                </div>
              </div>
              ...
              <CardContent className="space-y-3">
                <div className="text-sm">
                  ...
                  {compatibleProducts.length > 0 ? (
                    ...
                    <p className="text-xs text-muted-foreground">No products assigned yet</p>
                  )}
                </div>
