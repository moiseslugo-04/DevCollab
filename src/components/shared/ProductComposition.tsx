...
import { Upload, CheckCircle, Settings, Send, Download } from 'lucide-react'
...
export default function ProductComposition() {
  const { currentStep, setCurrentStep, next, prev, progress } = useOrderStep(3)
  ...
  const stepsTitles = [
    ...
    { number: 2, name: 'Choose Options', icon: Settings },
    { number: 3, name: 'Review & Submit', icon: Send },
  ]
  ...
  return (
    <div>
      ...
      {currentStep === 1 && productStep.canProceed && (
              ...
              <div className='flex items-center justify-between'>
                <div>
                  ...
                  <p className='text-sm text-blue-700'>
                    Download the correct gabarito for{' '}
                    {productStep.selectedProduct.name}
                  </p>
                </div>
                <Button
                  size='sm'
      ...
      {/* Step 3: Finishing Options */}
      {currentStep === 2 && artworkStep.canProceed && (
        <FinishingOptions
          {...finishingStep}
      ...
      {currentStep === 3 && finishingStep.canProceed && (
        <Review
          {...reviewStep}
          prev={prev}
          product={product}
